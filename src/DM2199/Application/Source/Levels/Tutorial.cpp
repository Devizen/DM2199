...
void Tutorial::Init()
{
    ...
    if (Tutorial_Inventory == 0)
    {
        Tutorial_Inventory = new Inventory();
        Tutorial_Inventory->setdefined(1);
    }
    ItemInfo* sword = new ItemInfo("sword", 20, 20, 10, 5);
    Tutorial_Inventory->addItem(sword);

    ItemInfo* torch = new ItemInfo(("torch"), 0, 50, 0, 10);
    Tutorial_Inventory->addItem(torch);
    // adding items to inventory
    ItemInfo* fist = new ItemInfo();
    Tutorial_Inventory->addItem(fist);
    ...
    SceneBase* enemyRender = new SceneBase();

    //speed ,damage ,hp, range ,this
    Enemy * newEnemy = new Spider(70.f, 5.f, 50.f, 100.f, enemyRender);
    newEnemy->addWaypoint(Vector3(200, 0, 200));
    newEnemy->addWaypoint(Vector3(200, 0, -200));
    newEnemy->addWaypoint(Vector3(-200, 0, -200));
    newEnemy->addWaypoint(Vector3(-200, 0, 200));
    ...
    // speed , damage ,hp , range , this , int(used to identify which type of robot)
    Enemy * newEnemy2 = new Robot(70.f, 5.f, 100.f, 100.f, enemyRender, 1);
    newEnemy2->addWaypoint(Vector3(300, 0, -100));
    newEnemy2->addWaypoint(Vector3(400, 0, -400));
    newEnemy2->addWaypoint(Vector3(-400, 0, -400));
    newEnemy2->addWaypoint(Vector3(-400, 0, 400));
    ...
    Enemy * newEnemy3 = new Spider(70.f, 5.f, 50.f, 100.f, enemyRender);
    newEnemy3->addWaypoint(Vector3(-400, 0, 200));
    newEnemy3->addWaypoint(Vector3(300, 0, -300));
    newEnemy3->addWaypoint(Vector3(-2300, 0, -300));
    newEnemy3->addWaypoint(Vector3(-2300, 0, 200));
...
void Tutorial::Update(double dt)
{
    ...
    SceneBase* bulletRender = new SceneBase();
    ...
    if (Application::IsKeyPressed(MK_LBUTTON))
    {
        if (_elapsedTime >= nextBulletTime)
        {
            objFactory.createFactoryObject(new Bullet(bulletRender, camera.getPosition(), camera.getYaw(), camera.getPitch()));
            nextBulletTime = _elapsedTime + coolDown;
            soundStorage[0]->play3DSound(false, false, false, camPos);
            bulletTouch = false;
        }
...
void Tutorial::enemyUpdate(double dt)
{
	...
	for (vector<Enemy*>::iterator it = enemyStorage.begin(); it != enemyStorage.end(); it++)
	{
		...
		{
			if ((*factoryIt)->type == Tutorial::GEO_LIGHTBALL3)   //BULLET INTERACTION WITH ENEMY
			{
...
void Tutorial::renderInventory()
{
    ...
    if (Tutorial_Inventory->getActiveItem())
    {
        ItemInfo* activeItem = Tutorial_Inventory->getActiveItem();
    ...
    if (Tutorial_Inventory->getSecondaryItem())
    {
        ItemInfo* secondaryItem = Tutorial_Inventory->getSecondaryItem();
    ...
    ItemInfo* ItemDisplay1 = Tutorial_Inventory->getDisplay1();
    ...
    ItemInfo* ItemDisplay2 = Tutorial_Inventory->getDisplay2();
...
void Tutorial::renderText()
{
<<<<<<< e903a2d4b6edc3901712d8910c89527aa38a81c0
	std::string sFPS = "FPS: " + std::to_string(FPS);
	sFPS.resize(9);
	RenderTextOnScreen(meshList[GEO_FPS], sFPS, Color(1, 1, 0), 2, 0, 29);
}
